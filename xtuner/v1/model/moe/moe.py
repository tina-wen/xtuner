        ...
        if self.mtp_block is not None:
            ...
            for micro_batch_idx, (loss_ctx_dict, mtp_outputs) in enumerate(zip(loss_ctx_list, mtp_outputs_per_mb)):
                ...
                for mtp_idx, (mtp_hidden, mtp_ctx) in enumerate(zip(mtp_outputs, mtp_loss_ctx_list)):
                    ...
                    mtp_loss, _ = self.lm_head(mtp_hidden_states, cast(MTPLossContext, mtp_ctx), skip_all_reduce=skip_all_reduce)
            ...
            if has_mtp_loss:
                if skip_all_reduce:
                    ...
                    if dist.is_initialized() and scaled_mtp_loss.requires_grad:
                        scaled_mtp_loss = torch.distributed.nn.functional.all_reduce(scaled_mtp_loss, op=dist.ReduceOp.SUM, group=dist.group.WORLD) 
